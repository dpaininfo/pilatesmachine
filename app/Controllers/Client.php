...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public function reserver($date = null, $heure = null)
	{
		...
		elseif (date('Y-m-d H:m', strtotime($date.' '.$heure)) <= date('Y-m-d H:m'))
		{
			$data['erreur'] = 'passé';
		}
		//si la séance n'existe pas
		elseif ($modSeance_Machine->SeanceExistante($nodays[date('l', strtotime($date))], $heure) == null || (date('F', strtotime($date)) == 'July' || date('F', strtotime($date)) == 'August'))
		{
			$data['erreur'] = 'inexistant';
		}
		//s'il n'y a plus de place pour cette séance 
		elseif ($modinscriptions->GetNombreInscription($date, $heure) == $nbinscriptionsmax)
		{
			$data['erreur'] = 'places';
		}
		//si l'abonnement est expiré
		elseif (date("Y-m-d H:i",strtotime($date.' '.$heure)) > $datefinabonnement->format('Y-m-d H:i'))
		{
            $data['erreur'] = 'expire';
		}
		//si pas assez de séances sur l'abo
		elseif ($abonnementMachine->NBSEANCESRESTANTES == 0)
		{
			$data['erreur'] = 'nbseances';
		}
		//si la personne a appuyé sur 'S"inscrire" ou "Réserver" pour valider en vue d'enregistrement BD
		elseif (isset($_POST['btnInscription']))
		{
			// après répétition
					...
					else
					{
						for ($i = $origin; $i <= $target; $i->modify('+7 day'))
						{
							...
							if ($datefinabonnement->format('Y-m-d') <= $i->format('Y-m-d'))
							{
								...
								{
								if (($places == null || $places < $nbinscriptionsmax) && $data['estinscrit'] == null)
								{
			...
			else
			{
				...
				if ($estinscrit == null)
				{
                    $dateDesinscMax = new DateTime(date('Y-m-d H:i', strtotime(($date.' '.$heure))));
                    $dateDesinscMax->modify('-2 day');
					$donneesAInserer = array(
						'NOABONNEMENT' => $abonnementMachine->NOABONNEMENT,
						'NOJOUR' => $nodays[date('l', strtotime($date))],
						'HEUREDEBUTSEANCE' => $heure,
						'DATESEANCE' => $date,
                        'DATEHEUREDESINSCRIPTION' =>  $dateDesinscMax->format('Y-m-d H:i'),
					);
        ...
        // ajout de la date desinscription
        ...
        public function validerReservation()
        {
            ...
            elseif (isset($_POST['btnReserver']))
            {
                foreach ($seances as $laseance)
                {
                    ...
                    $dateDesinscMax = new DateTime(date('Y-m-d H:i', strtotime($laseance.' '.$this->request->getPost('heure'.$laseance))));
                    $dateDesinscMax->modify('-2 day');
                    if ($estinscrit == null)
                    {
                        $donneesAInserer = array(
